...
C
C     WRITE(STR,*) 'ADWDM:UTWDMD:WDRCGO',wdmsfl,rrec
C     CALL LOG_MSG(STR)
C
      RIND = 0
      CWDM = 0
...
C
C     write(STR,*) 'ADWDM:UTWDMD:WDRCGO2',cwdm,donfg,wdmcnt
C     CALL LOG_MSG(STR)
C     write(STR,*) 'ADWDM:UTWDMD:WDRCGO3',maxrec(cwdm),wdmopn(cwdm)
C     CALL LOG_MSG(STR)
C
      IF (DONFG.EQ.1) THEN
...
C           record not found, allocate space for it
            RIND= FREPOS
            IF (RREC.GT.0) THEN
              INQUIRE(UNIT=WDMSFL,NAME=WDNAME,OPENED=OPEN)
              IF (OPEN) THEN
C               read existing record from file
                READ (WDMSFL,REC=RREC) (WIBUFF(I,RIND),I=1,512)
              ELSE
                WRITE(STR,*) 'UTWDMD:NotOpen:',WDMSFL,RREC,TRIM(WDNAME)
C               CALL LOG_MSG(STR)
              END IF
            ELSE
...
C       we have got a bad problem
        BADCNT = BADCNT + 1
        IF (BADCNT.LT.50) THEN
C         restrict the number of times this message is written
C         to avoid filling up the pipe
          WRITE(*,*) 'ADDWM:UTWDMD:WDRCGO:',WDMCNT,CWDM,DONFG,
     #                MAXREC(CWDM),RREC
          WRITE(*,*) '               want:',WDMSFL,' open:',
     #                (WDMOPN(I),I=1,WDMCNT)
        ELSEIF (BADCNT.EQ.50) THEN
          WRITE(*,*) 'ADDWM:UTWDMD:WDRCGO: write no more messages'
        END IF
        RIND = 1
      END IF
C
...
C
C     + + + HISTORY + + +
C     2005/06/30 JLK allow message file to be closed
C
C     + + + DUMMY ARGUMENTS + + +
      INTEGER   WDMSFL,RETCOD
C
...
C
      RETCOD= 0
C
C     WRITE(99,*,ERR=10) 'WDFLCL:entry',WDMSFL,
C    #            ' count',WDMCNT,
C    #            ' open:',(WDMOPN(I),I=1,WDMCNT)
C10   CONTINUE
C
C     loop to thru open files
      I = 1
      DO WHILE (I .LE. WDMCNT)
        IF (WDMOPN(I) .NE. WDMSFL) THEN
C         try next one
          I = I+ 1
        ELSE
C         this file is open
...
C         TODO: how slow is this?
C         CALL WDMCHK(WDMSFL,RETCOD)
...
C         force exit from loop
      ...
      END DO
C
C     WRITE(99,*,ERR=40) 'WDFLCL:exit:',WDMSFL,
C    #            ' count',WDMCNT,
C    #            ' open:',(WDMOPN(I),I=1,WDMCNT)
C40   CONTINUE
C
...
C     + + + END SPECIFICATIONS + + +
C
C     INQUIRE(UNIT=99,OPENED=OPN)
C     IF (OPN) WRITE(99,*) 'ADWDM:UTWDMD:WDDSCK',wdmsfl,dsn
C
      RETCOD= 0
...
C
C       IF (OPN) WRITE(99,*) 'ADWDM:UTWDMD:WDDSCK:dirrec',dirrec
C
        IF (DIRREC.GT.0) THEN
...
C         no directory, dsn cant exist
        ...
        END IF
C       IF (OPN) THEN
C         WRITE(99,*) 'ADWDM:UTWDMD:WDDSCK:',WDMSFL,DSN,DIRREC,DREC
C       END IF
C
        IF (DREC.EQ.0) THEN
C         data set does not exist
